...
```{r passenger ,fig.align='center'}

p <- ggplot(data=s_table1, aes(x=`Passenger class`, y=Count)) +
...
```{r passengerP , fig.cap="Composition of the passengers by passenger class (Sample size = 891)"}



p<-ggplot(data=s_table1, aes(x=`Passenger class`, y=`Percentage`)) +
  geom_bar(stat="identity", fill = "lightblue") +
...
p <- ggplot(data=datas, aes(x=Pclass, y=n, fill=Survived)) +
