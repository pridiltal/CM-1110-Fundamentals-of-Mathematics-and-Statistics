...
\item
  The steepness of the graph of a function (\(f\)) at a given point on it (say \((x_0, f{(x_0)})\)), can be approximate by its \textbf{tangent Line} at that point.
\item
...
\textbf{Definition: Tangent line }

\textbf{The tangent line to the curve \(y=f(x)\) at the point \(P(x_0, f(x_0))\) is the line through \(P\) with slope} \[m = lim_{x\to x_{0}}\frac{f(x)-f(x_0)}{x-x_0}\]
...
\tightlist
\item
  When the graph of \(f\) has a vertical tangent line at a given point. (\(\frac{f(x) - f(x_0)}{x-x_0}\) tends to \(\infty\) or to \(-\infty\) as \(x\) tends to \(x_0\))
\end{enumerate}
...
\tightlist
\item
  When there is a jump discontinuity. The graph of a function will not have a tangent line at a point where the function is discontinuous.
\end{enumerate}
...
\hypertarget{the-derivative-of-xn-derivatives-of-linear-combinations}{%
\subsection{\texorpdfstring{The derivative of \(x^n\), derivatives of linear combinations}{The derivative of x\^{}n, derivatives of linear combinations}}\label{the-derivative-of-xn-derivatives-of-linear-combinations}}

\hypertarget{the-product-and-quotient-rules}{%
\subsection{The product and quotient rules}\label{the-product-and-quotient-rules}}

\hypertarget{average-velocity-and-velocity}{%
\subsection{Average velocity and velocity}\label{average-velocity-and-velocity}}

\hypertarget{other-rate-of-change}{%
\subsection{Other rate of change}\label{other-rate-of-change}}

\hypertarget{composite-functions-and-the-chain-rule}{%
\subsection{Composite functions and the chain rule}\label{composite-functions-and-the-chain-rule}}

\hypertarget{the-derivatives-of-sine-and-cosine}{%
\subsection{The derivatives of sine and cosine}\label{the-derivatives-of-sine-and-cosine}}
...
\hypertarget{indefinite-integrals-and-integrals-of-xn-nneq-1-sin-x-cos-x}{%
\subsection{\texorpdfstring{Indefinite integrals and integrals of \(x^n\) (\(n\neq-1\), \(sin x\), \(cos x\))}{Indefinite integrals and integrals of x\^{}n (n\textbackslash{}neq-1, sin x, cos x)}}\label{indefinite-integrals-and-integrals-of-xn-nneq-1-sin-x-cos-x}}
...
\addcontentsline{toc}{subsection}{Chapter 5.1: Differentiation}
...
\caption{(a)}\label{fig:unnamed-chunk-11}
\end{figure}
...
\caption{(b)}\label{fig:unnamed-chunk-12}
\end{figure}
...
\caption{Relationship of mean, median and mode for different types of skewness}\label{fig:unnamed-chunk-33}
\end{figure}
...
\caption{Types of kurtosis}\label{fig:unnamed-chunk-34}
\end{figure}
